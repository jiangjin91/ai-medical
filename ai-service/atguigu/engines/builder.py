...
def build_dialogue_engine():
    ...
    return DialogueEngine(
        ...
        knowledge_handler=KnowledgeHandler(knowledge_intents=KNOWLEDGE_INTENTS,
                                           knowledge_register=KnowledgeRegister([ApiOrderProvider(),ApiProductProvider(),RagDefaultProvider(),FaqDefaultProvider()]),
                                           knowledgeResponder=KnowledgeResponder()),    # 知识意图处理器
        chitchat_handler=ChitChatHandler(ChitChatResponder())                           # 闲聊处理器
    )