...
class TurnPlan:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "TurnPlan":
        return cls(
            task = TaskTurnPlan.from_dict(data["task"]) if data["task"] is not None else None,
            knowledge = KnowledgeTurnPlan.from_dict(data["knowledge"]) if data["knowledge"] is not None else None,
            chitchat = ChitChatTurnPlan.from_dict(data["chitchat"]) if data["chitchat"] is not None else None
        )
