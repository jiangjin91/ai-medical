...
class TurnPlanValidator:
    ...
    def _validate_task_track(self, task: TaskTurnPlan, flow_list: FlowList) -> TurnPlanValidatedResult:
        ...
        # 3.是否有多个开启command命令
        start_comand = [command for command in task.commands if isinstance(command,StartFlowCommand)]
        if len(start_comand)>1:
            return self._reject(ClarifyReason.MULTIPLE_TRACKS)

        if start_comand:
            flow_id = start_comand[0].flow
    ...
    def _validate_knowledge_track(self, knowledge: KnowledgeTurnPlan, dialogue_state: DialogueState,
